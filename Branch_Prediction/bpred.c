...
#include "machine.h"
#include "bpred.h"
int L[8] = {0,3,5,8,12,19,31,49};    /*  */
int num_bits1[8] = {15,15,7,7,7,7,7,7};
int num_bits2[8] = {-16,-16,-8,-8,-8,-8,-8,-8};
int min(int a,int b)
{
	int c = a<b?a:b;
	return c;
	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredTournament:
    pred->dirpred.tournament = 
      bpred_dir_create(class, l1size, l2size, shift_width, 0);
    break;  
  
  /* OGEHL predictor component */
  case BPredOgehl:
    pred->dirpred.ogehl = 
      bpred_dir_create(class, l1size, l2size, 0, 0);
    break;   
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPredTournament:
  {
  	pred_dir->config.tournament.local_history_size = l1size;   //initializing local history size to 1024
  	pred_dir->config.tournament.local_pht_size = l1size;       //initializing local predictor size to 1024
  	pred_dir->config.tournament.global_table_size = l2size;    //initializing global predictor size to 4096
  	pred_dir->config.tournament.meta_table_size = l2size;      //initializing meta predictor size to 4096
  	pred_dir->config.tournament.ghr = 0;                       //initializing ghr value to zero
  	pred_dir->config.tournament.local_history = calloc(l1size,sizeof(unsigned int));	//allocating memory and intializing to zero for local history table
  	pred_dir->config.tournament.local_pht = calloc(l1size,sizeof(unsigned int));	//allocating memory and intializing to zero for local predictor table
  	pred_dir->config.tournament.global_table = calloc(l2size,sizeof(unsigned int));	 //allocating memory and intializing to zero for global history table
  	pred_dir->config.tournament.meta_table = calloc(l2size,sizeof(unsigned int));	 //allocating memory and intializing to zero for meta table
  	break;
  }  
  case BPredOgehl:
  {
  	int i;
  	for(i=0;i<49;i++)
	    	pred_dir->config.ogehl.ghr[i] = 0;         //initializing global history value to zero
  	pred_dir->config.ogehl.table = calloc(8,sizeof(int *));	 //allocating pointer for 8tables
  	for(i=0;i<8;i++)
  	{
  		pred_dir->config.ogehl.perfect_index[i] = 0;
  		if(i==1)
  		{
  			pred_dir->config.ogehl.table[i] = calloc(l1size,sizeof(int));	   //allocating memory(1K) and initializing to zero
  			continue;	
  		}
  		pred_dir->config.ogehl.table[i] = calloc(l2size,sizeof(int));	    //allocating memory(2K) and initializing to zero
  	}
  	for(i=0;i<16;i++)
  	{
  		pred_dir->config.ogehl.phist[i] = 0;	     //initializing path history value to zero
  	}
  	pred_dir->config.ogehl.sum=0;      //initializing sum value to zero
  	break;
  }	
  case BPredTaken:
  case BPredNotTaken:
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  /* Added torunament predictor case */
  case BPredTournament:
  ...
  /* Added ogehl predictor case */
  case BPredOgehl:
    bpred_dir_config (pred->dirpred.ogehl, "ogehl", stream);
    fprintf(stream, "btb: %d sets x %d associativity", 
	    pred->btb.sets, pred->btb.assoc);
    fprintf(stream, "ret_stack: %d entries", pred->retstack.size);
    break;
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
    ...
    //////////////////////////////////////////////////////////////////
    /*
    Lookup for tournament and ogehl predictor
    */
    //////////////////////////////////////////////////////////////////
    ...
    case BPredTournament:
    {
    	int high = 1;          //to return if taken
    	int low = -1;           //to return if not taken
    	int index = (baddr >> 2) & 0x3FF;        //to hold the 2-11 bits of program counter
    	int index2 = pred_dir->config.tournament.local_history[index];          //index of local history table
    	int local_result = pred_dir->config.tournament.local_pht[index2];      //value of single local predictor index selected
    	
    	int g_index = pred_dir->config.tournament.ghr & 0xFFF;       //index of global predictor table
    	int global_result = pred_dir->config.tournament.global_table[g_index];    //value of global predictor index selected
    	int sel = pred_dir->config.tournament.meta_table[g_index];        //selector value from meta table
    	if(sel<2)          //if select < 2 then select local predictor
    	{
    		if(local_result<4)    		    //if local table predictor value < 4 predict as not taken
    			q = (char *)&low;
    		
    		else    		
    			q = (char *)&high;         //if local table predictor value >= 4 predict as taken
    		
    	}
    	else                           //if select >= 2 then select global predictor
    	{
    		if(global_result<2)    		
    			q = (char *)&low;        //if global table predictor value < 2 predict as not taken
    		
    		else    		
    			q = (char *)&high;       //if global table predictor value >= 2 predict as not taken
    ...
    case BPredOgehl:
    {  
      //variables for calculating different indexes  	
    	int ghr_decimal=0,phist_decimal=0;
    	int index = (baddr >> 2) & 0xFF;
    	int final_index=0;
    	int i,nbits,j,counter=0,counter1=0,var;
    	float temp,l=0.0;
    	int *bits,*bits1;
    	pred_dir->config.ogehl.sum=0;

      /* iterating over 8 tables for predicted values */
    	for(i=0;i<8;i++)
    	{
    		var = min(L[i],16);    //calculating the min of L(i) and 16 to select number of bits in path history
    		nbits = L[i] + 8 + var;     //number of bits calculated for indexing
    		/* if not first table */
        if (i!=1)       
    		{
          /* if number of bits greater than 3n implementing hash function */
    			if (nbits > 33)     
    			{	
            /* assigning the bits from phist, PC and ghr */
    				bits = calloc(nbits,sizeof(int));
    				for(j=0;j<nbits;j++)
    				{
              /* assigning phist bits to LSB */
    					if(j < var)
    					{
    						bits[j] = pred_dir->config.ogehl.phist[j]; 
    					}
              /* assigning branch address bits */
    					else if(j>=var && j<var+8)
    					{						
    						bits[j] = (index >> counter++) & 0x1;
    					}
              /* assigning ghr bits */  
    					else
    					{
    						bits[j] = pred_dir->config.ogehl.ghr[counter1++];
    					}
    				}
    				temp = (float)nbits / (float)33.0;
    				counter=0;
    				bits1 = calloc(33,sizeof(int));	
    				bits1[0] = bits[0];

            /* hashing function to return 3n bits out of calculated number of bits */ 
    				for(j=1;j<33;j++)
    				{
    					l = l + temp;
    					bits1[j] = bits[(int)l];
            ...
            /* final index to be refered to table (XOR operation) */
    				for(j=0;j<11;j++)
    				{
    					final_index += pow(2,j) * (bits1[j] ^ bits1[j+11] ^ bits1[j+22]);
    				}

            /* storing the final index */
    				pred_dir->config.ogehl.perfect_index[i] = final_index;

            /* calculating sum with the counter value of the index of the table */
    				pred_dir->config.ogehl.sum = pred_dir->config.ogehl.sum + pred_dir->config.ogehl.table[i][final_index];
    				free(bits);
    				free(bits1);
    				counter=0;
    				counter1=0;
          ...
          /* if number of bits not greater than 3n */
    			else
    			{
            /* calculating the ghr and phist values */
    				for(j=0;j<11;j++)
    				{
    					ghr_decimal += pow(2,j) * pred_dir->config.ogehl.ghr[j];
    					phist_decimal += pow(2,j) * pred_dir->config.ogehl.phist[j];
    				}
            /* final index to be refered to table (XOR operation) */			
    				final_index = ((phist_decimal & (int)(pow(2,L[i])-1)) ^ index) ^ (ghr_decimal & (int)(pow(2,L[i])-1));
          ...
          /* if number of bits greater than 3n implementing hash function */
            ...
            /* assigning the bits from phist, PC and ghr */
              ...
              /* assigning ghr bits */
    				...
    				temp = (float)nbits / (float)30.0;
    				counter=0;
    				bits1 = calloc(30,sizeof(int));	
    				bits1[0] = bits[0];
            ...
            /* final index to be refered to table (XOR operation)*/
    				for(j=0;j<10;j++)
    				{
    					final_index += pow(2,j) * (bits1[j] ^ bits1[j+10] ^ bits1[j+20]);
    				}
            /* storing the final index */
    				pred_dir->config.ogehl.perfect_index[i] = final_index;
    	...
    	return ((char *)&(pred_dir->config.ogehl.sum));       	//returning the sum calculated	 		
    	break;
    }  
    case BPred2bit:
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
    ...
    case BPredTournament:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
    	{
    	  dir_update_ptr->lab = bpred_dir_lookup (pred->dirpred.tournament, baddr);
    	}
    break;

    /* OGEHL branch predictor */
    case BPredOgehl:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
    	{
    	  dir_update_ptr->lab = bpred_dir_lookup (pred->dirpred.ogehl, baddr);
    	}
    break;  
    case BPredTaken:
      return btarget;
  ...
  //////////////////////////////////////////////////////////////////
    /*
    Return for tournament and ogehl predictor
    */
  //////////////////////////////////////////////////////////////////

  /* return taken/not taken and branch address for Tournament and OGEHL branch predictors */
  if(pred->class == BPredOgehl || pred->class == BPredTournament)
  {
  	if (pbtb == NULL)
  	{	
      /* BTB miss - return predicted direction */
  		return ((*(dir_update_ptr->lab)<0)?0:1);     //0 - not taken; 1 - taken
  	}
  	else
  	{
  	   /* BTB hit - return target branch if taken */
  		return ((*(dir_update_ptr->lab)<0)?0:pbtb->target);
  	}
  }  
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if(dir_update_ptr->lab)
  {
    //////////////////////////////////////////////////////////////////
    /*
    My updation algorithm for tournament and ogehl predictor
    */
    //////////////////////////////////////////////////////////////////
    ...
    /* Tournament branch predictor */
  	...
  	{    		
  		int index = (baddr>>2) & 0x3FF;     //branch address

      /* 12bits from ghr */
  		pred->dirpred.tournament->config.tournament.ghr = pred->dirpred.tournament->config.tournament.ghr & 0xFFF;

      /* if mis-prediction */
  		if(!correct)
	    {
        /* if taken incrementing meta table value on condition meta table value<3 as meta table is of 2bit */
	      if(taken && pred->dirpred.tournament->config.tournament.meta_table[pred->dirpred.tournament->config.tournament.ghr]<3)
		      pred->dirpred.tournament->config.tournament.meta_table[pred->dirpred.tournament->config.tournament.ghr]++;
	      
        /* if not taken decrementing meta table value on condition meta table value >0 as meta table is of 2bit */
	      if((!taken) && pred->dirpred.tournament->config.tournament.meta_table[pred->dirpred.tournament->config.tournament.ghr]>0)
		      pred->dirpred.tournament->config.tournament.meta_table[pred->dirpred.tournament->config.tournament.ghr]--;     
	    }		    
	    
      /* index of local history table */
     int second_index = pred->dirpred.tournament->config.tournament.local_history[index];

      /* if taken incrementing local predictor value on condition <7 as it is of 3bit */
      if(taken && pred->dirpred.tournament->config.tournament.local_pht[second_index]<7)
       pred->dirpred.tournament->config.tournament.local_pht[second_index]++;
      
      /* if not taken decrementing local predictor value on condition >0 as it is of 3bit */
      else if((!taken) && pred->dirpred.tournament->config.tournament.local_pht[second_index]>0)
	      pred->dirpred.tournament->config.tournament.local_pht[second_index]--;  		    

      /* local history => left shift by one and increment by 1 if taken; left shift by one if not taken */
      pred->dirpred.tournament->config.tournament.local_history[index] = pred->dirpred.tournament->config.tournament.local_history[index] & 0x3FF;    
    	pred->dirpred.tournament->config.tournament.local_history[index] = pred->dirpred.tournament->config.tournament.local_history[index]<<1; 

      if(taken)
	       pred->dirpred.tournament->config.tournament.local_history[index]++; 
      
       /* if taken incrementing ghr value on condition ghr value<3 as ghr is of 2bit */
      if(taken && pred->dirpred.tournament->config.tournament.global_table[pred->dirpred.tournament->config.tournament.ghr]<3)
	       pred->dirpred.tournament->config.tournament.global_table[pred->dirpred.tournament->config.tournament.ghr]++;
      
      /* if not taken decrementing ghr value on condition ghr value>0 as ghr is of 2bit */
      if((!taken) && pred->dirpred.tournament->config.tournament.global_table[pred->dirpred.tournament->config.tournament.ghr]>0)
	      pred->dirpred.tournament->config.tournament.global_table[pred->dirpred.tournament->config.tournament.ghr]--; 

      /* ghr => left shift by one and increment by 1 if taken; left shift by one if not taken */
      pred->dirpred.tournament->config.tournament.ghr = pred->dirpred.tournament->config.tournament.ghr << 1;
      if(taken)
	       pred->dirpred.tournament->config.tournament.ghr++; 		
    ...
    /* OGEHL branch predictor */
  	...
  	{   	
  		/* if mis-prediction and absolute value of sum < threshold (threshold = number of tables = 8) */
  		if((!correct) || abs(pred->dirpred.ogehl->config.ogehl.sum) < 8)
  		{    			
	    	int i,p,q;

        /* updating counter values for all tables */
	    	for(i=0;i<8;i++)
	    	{
	    		q = pred->dirpred.ogehl->config.ogehl.table[i][pred->dirpred.ogehl->config.ogehl.perfect_index[i]];
	    		p = num_bits2[i];

          /* if taken and counter value is < max value(7-4 bit, 15 - 5 bit) then incrementing counter */		    						    		
    			if(taken && pred->dirpred.ogehl->config.ogehl.table[i][pred->dirpred.ogehl->config.ogehl.perfect_index[i]]<num_bits1[i])
    				pred->dirpred.ogehl->config.ogehl.table[i][pred->dirpred.ogehl->config.ogehl.perfect_index[i]]++;
    			else if(!taken)   /* if taken and counter value is > min value(-8 for 4 bit, -16 for 5 bit) then decrementing counter */                           
    			{
    				if(q>p)					
    				pred->dirpred.ogehl->config.ogehl.table[i][pred->dirpred.ogehl->config.ogehl.perfect_index[i]]--;
    			}						
      ...
      /* shifting the ghr value */
  		for(i=48;i>0;i--)
    	{
    		pred->dirpred.ogehl->config.ogehl.ghr[i] = pred->dirpred.ogehl->config.ogehl.ghr[i-1];
    	}

      /* if taken push 1 to ghr */
    	if(taken)
    	{
    		pred->dirpred.ogehl->config.ogehl.ghr[0] = 1;
    	}    
    	else     /* if not taken push 0 to ghr */
    	{
    		pred->dirpred.ogehl->config.ogehl.ghr[0]=0;
    	}

      /* shifting the path history */
    	for(i=15;i>0;i--)
    	{
    		pred->dirpred.ogehl->config.ogehl.phist[i]=pred->dirpred.ogehl->config.ogehl.phist[i-1];
    	}

      /* push LSB of branch address to path history */
    	pred->dirpred.ogehl->config.ogehl.phist[0] = (baddr >> 2) & (0x1);	
  	}
