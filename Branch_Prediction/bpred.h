...
struct bpred_dir_t {
  ...
  union {
    ...
    struct {
      unsigned int local_history_size;     /* local history table size */
      unsigned int local_pht_size;        /* single local predictor table size */
      unsigned int global_table_size;   /* Global predictor table size */
      unsigned int meta_table_size;     /* Meta predictor table size */
      unsigned int ghr;                 /* Global history */
      unsigned int *local_history;      /* local history table */
      unsigned int *local_pht;          /* single local predictor table */
      unsigned int *global_table;       /* Global predictor table */
      unsigned int *meta_table;    	    /* Meta predictor table */
    }tournament;
    struct {
      int ghr[49];          /* Global history register */
      int **table;          /* 8 table components */
      int phist[16];        /* path history */
      int sum;	           /* sum of entry selected from all tables */
      int perfect_index[8];     /* To hold the index to be indexed from each table */
    }ogehl;
  } config;
};
...
struct bpred_t {
  ...
  struct {
    ...
    struct bpred_dir_t *ogehl;      /* OGEHL predictor */
  } dirpred;
