  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      if (cp->hsize)
	{
	 ...
	 during random replacement selection) */
      ...
      /* Initializing the bits tree structure for PLURt */
      int count;
      int level = log_base2(cp->assoc);    //to calculate levels in binary tree
      cp->sets[i].bits = calloc(level,sizeof(int *));   //initializing the bits to 0
      for(count = 0; count < level; count++)
      {
        cp->sets[i].bits[count] = calloc(pow(2,count),sizeof(int));
      }
    ...
    blk->rrpv_counter = 3;   //setting the rrpv counter to 3 initially
	  blk->status = 0;
	  blk->tag = 0;
	  blk->ready = 0;
...
/* To get the index of the replacement line */
int get_index(int associativity, int **tree)
{
  int index = 0, i, prev_index = 0;
  int levels = log_base2(associativity);   //levels in a binary tree
  for(i = 0; i < levels; i++)
  {
    if(i == 0)      //calculating the index of replacement line if the node value is 0
      index = index + pow(2,levels-i-1)*tree[0][0];
    else        //calculating the index of replacement line if the node value is 1
    {
        prev_index = (prev_index << 1) + tree[i-1][prev_index];
        index = index + pow(2,levels-i-1)*tree[i][prev_index];
    }   
  }
  return index;  
}

/* To update the nodes of tree once cache access is done */
int ** update_bits(int associativity, int index, int **tree)
{ 
  int i;
  int levels = log_base2(associativity);     //levels in a binary tree
  int temp_index = index >> 1;          //replaced index right shift
  for(i = 0; i < levels; i++)         //updating the bits(toggling) of the tree as per replaced block
  {
    if(tree[levels-i-1][temp_index] == 0)     //if node value = 0
      tree[levels-i-1][temp_index] =  1;
    else                                    //if node value = 1
      tree[levels-i-1][temp_index] = 0;     
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case PLRUt:
  {
    int **temp = cp->sets[set].bits;     //nodes values of tree
    int index = get_index(cp->assoc, temp);     //to get the index of the line to be replaced
    repl = CACHE_BINDEX(cp, cp->sets[set].blks, index);     //replacement line
    cp->sets[set].bits = update_bits(cp->assoc, index, temp);   //update the node values after cache access
  }
  break;
  case SRRIP:
  {
     repl = NULL;
      while(repl == NULL){
        element = cp->sets[set].way_head;
        while(element != NULL)
        {      // searching for rrpv value=3 form left to right
          if(element->rrpv_counter == 3){   // to find the block to be replaced
            repl = element;     //replaced block as rrpv value = 3
            repl->rrpv_counter = 2;   /* setting the replaced block's rrpv value to 2 */
            break;
          }else
            element = element->way_next;   //traverse until rrpv value = 3 is found
        }
        if(repl == NULL){               /* to increment all rrpv values as rrpv=3 is not found */
          element = cp->sets[set].way_head;      //set head pointer
          while(element != NULL){
            element->rrpv_counter++;      //incrementing all rrpv values
            element = element->way_next;
          }
        }
      }      
  }
 ...
 cache_hit: /* slow hit handler */
  ...
  if (blk->way_prev && cp->policy == LRU)
    ...
    /* PLURt replacement policy and update bits once hit */
  if(cp->policy == PLRUt){
      int index = 0;
      struct cache_blk_t *test_blk = blk;
      for (test_blk = blk; test_blk != NULL; test_blk = test_blk->way_prev)        //get the indexof the hit block
      {
        index ++;
      }
      int **temp = cp->sets[set].bits;
      cp->sets[set].bits = update_bits(cp->assoc, index, temp);     //update the node values of the tree
  }

  /* setting rrpv value to 0 once cache hit */
  blk->rrpv_counter = 0;
